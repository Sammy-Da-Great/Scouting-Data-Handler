...
class Global_Config():
    def __init__(self, host, user, password, database_name, tba_key): 
        self.host = host
        self.user = user
    ...
    def __str__(self):
        return "{0}, {1}, {2}, {3}".format(self.host, self.user, self.password, self.database_name, self.tba_key)
...
def initialize_configs():
    ...
    if not(Path("config/global_config.json").is_file()):
        make_config(Global_Config("localHost", "defaultUser", "defaultPassword", "defaultDatabaseName", "TBA_Key"), "global_config.json")
